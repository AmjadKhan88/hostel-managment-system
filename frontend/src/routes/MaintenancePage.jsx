...
import MaintenanceFormModal from '@/features/maintenance/components/MaintenanceFormModal.jsx';
...
export default function MaintenancePage() {
  ...
  const { data, isLoading, isError, error } = useMaintenanceTickets({
  ...
  if (!effectiveHostelId) {
    return (
      <EmptyState
        ...
        description="Use the hostel switcher in the top bar to pick or create a hostel before managing maintenance."
      />
    );
  }
  ...
  const columns = [
    ...
    { key: 'room', header: 'Room', render: (row) => row.roomId?.roomNumber ?? '—' },
    ...
    { key: 'assignedTo', header: 'Technician', render: (row) => row.assignedTo?.name ?? '—' },
  ...
  return (
    <div className="mx-auto max-w-6xl">
      <PageHeader
        title="Maintenance"
        description="Track maintenance tickets across rooms and assets."
        action={
          <button
      ...
      <MaintenanceFormModal open={modalOpen} onClose={() => setModalOpen(false)} hostelId={effectiveHostelId} />
    </div>
  );
}